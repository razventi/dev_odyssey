---
title: "폴리스너츠 한글판 패치 — 다운로드 · 설치 가이드 (개정판)"
date: 2026-06-21
draft: true
...
> ✅ **이 방법은 검증되었습니다.** ImgBurn 합본을 분리하면 **데이터 트랙(Track 1)의 SHA-1이 정본과 정확히 일치**합니다(디스크 3장 모두 확인, `split.ps1`·binmerge 결과 동일). 데이터 트랙은 섹터 단위로 정렬되어 읽히므로, 오디오 오프셋과 무관하게 정본과 같아집니다.

> ℹ️ **오디오 트랙(Track 2) 참고:** 이 방법에서 오디오 트랙은 pregap(2초·150섹터) 처리 방식 차이로 redump 비트퍼펙트가 아닐 수 있습니다(약 4.45MB로 나옴). **게임 실행과 패치(데이터 트랙에만 적용)에는 전혀 지장이 없습니다.**
...
> 🛠 **간편 자동 검증:** 동봉 [`verify.ps1`](/downloads/tools/verify.ps1) 에 `.bin` 파일을 넘기면 정본/한글/오디오 표와 자동으로 대조해 `[OK]`/`[불일치]` 로 알려줍니다.
> ```
